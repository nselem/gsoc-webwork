...
############################################################################
		    ...
		    ##########################################
		    	...
		    	my $lang = $self->{envir}->{'language'};
		    	if(!defined $lang) { $lang = ''; }
		    	$lang = uc($lang);
		    	# BEGIN_TEXT
		    	my @ma = $evalString =~ m/BEGIN_TEXT(_[A-Z][A-Z_]*)?/g;
		    	my $mc = 0;
		    	my $i = 0;
		    	for(;$i<scalar @ma;++$i) {
		    		if(!defined $ma[$i]) { $ma[$i] = ''; }
		    		else { $ma[$i] = substr($ma[$i], 1); }
		    		if($ma[$i] eq $lang) { ++$mc };
		    	}
		    	if($mc == 0) { $lang = ''; }
		    	$i = 0;
		    	for(;$i<scalar @ma;++$i) {
		    		if($ma[$i] ne $lang) {
		    			if($ma[$i] eq '') { $evalString =~ s/BEGIN_TEXT[^_].+?END_TEXT//smg; }
		    			else { $evalString =~ s/BEGIN_TEXT_$ma[$i].+?END_TEXT//smg; }
		    		}
		    	}
		    	if($lang ne '') { $evalString =~ s/BEGIN_TEXT_$lang/BEGIN_TEXT/g; }

		    	# BEGIN SOLUTION
		    	@ma = $evalString =~ m/BEGIN_SOLUTION(_[A-Z][A-Z_]*)?/g;
		    	$mc = 0;
		    	$i = 0;
		    	for(;$i<scalar @ma;++$i) {
		    		if(!defined $ma[$i]) { $ma[$i] = ''; }
		    	...
		    	for(;$i<scalar @ma;++$i) {
		    		if($ma[$i] ne $lang) {
		    			if($ma[$i] eq '') { $evalString =~ s/BEGIN_SOLUTION[^_].+?END_SOLUTION//smg; }
		    			else { $evalString =~ s/BEGIN_SOLUTION_$ma[$i].+?END_SOLUTION//smg; }
		    		}
		    	}
		    	if($lang ne '') { $evalString =~ s/BEGIN_SOLUTION_$lang/BEGIN_SOLUTION/g; }
